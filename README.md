...
- **Clasp and name (off by default):** the same settings block has *Mosquetón* (lobster / heavy-duty carabiner) and *Nombre en letras* (A–Z plus the ✿ ☾ ★ ♥ charms, with size and position). Each can be hidden, shown for preview only, or sent with the order as a line-item property (`Mosquetón`, `Nombre en la correa`). Neither changes the price. When the product sells the clasp as a real option (setting *Opción del mosquetón*, default `Mosquetón`), that option drives the 3D like the finishes do and the tools pills disappear.
